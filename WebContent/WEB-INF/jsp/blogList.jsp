   ...
   ================================================== -->
							...
							<tr>
								<div id=blog${blog.b_id}>
									...
									<td style="text-align:center;"><a href="${pageContext.request.contextPath}/BlogController/blogEdit.action?b_id=${blog.b_id}" >编辑</a> | <a
										...
										onclick="delBlogById(${blog.b_id});">删除</a> <!-- delBlogById(${blog.b_id}); -->
									</td>
								</div>
							</tr>
						</c:forEach>
					...
					function delBlogById(b_id) {
						...
						$.ajax({
							...
							success:function(data){
								...
								//alert("#blog"+b_id);
								//$("#blog"+b_id).remove();
								window.location="${pageContext.request.contextPath}/BlogController/blogList.action"; 
							}
						}); 
					};
				</script>
	...
	<div class="row">
   		<nav class="pagination">
            ...
            <a href="${pageContext.request.contextPath }/BlogController/blogListBypage.action?page=${i}" ><span class="page-numbers current"><c:out value="${i}" /> </span> </a> 
            </c:forEach>  
	      </nav>
   	</div>
